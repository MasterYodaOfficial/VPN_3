...
class XUIHandler:
    ...
    async def add_client_vless(self, email: str) -> Optional[str]:
        """Асинхронное добавление VLESS-клиента."""
        try:
            # Если inbound_id не установлен, получаем инбаунды
    ...
    async def get_conf_user_vless(self, email: str, server_address: Optional[str] = None) -> Optional[str]:
    ...
    async def delete_client_vless(self, client_uuid: str) -> bool:
        ...
        try:
            ...
            if self.inbound_id is None:
                await self.get_inbounds()
                if self.inbound_id is None:
                    print("No VLESS inbound found for deletion")
                    return False
...
async def create_multiconfig(servers: List[dict], email: str) -> str:
    """
    Создает пользователей на нескольких серверах и возвращает мультиконфиг.

    Args:
        servers: Список словарей с параметрами серверов
        email: Email пользователя для создания

    Returns:
        Строка с объединенными конфигурациями
    """
    configs = []
    ...
    for server in servers:
        handler = XUIHandler(server["url"], server["user"], server["pass"])
        task = asyncio.create_task(_process_server(handler, email))
        tasks.append(task)

    # Ожидаем завершения всех задач
    results = await asyncio.gather(*tasks, return_exceptions=True)

    # Собираем успешные конфиги
    for result in results:
        if isinstance(result, Exception):
            print(f"Ошибка обработки сервера: {str(result)}")
        elif result:
...
async def _process_server(handler: XUIHandler, email: str) -> Optional[str]:
    """Обрабатывает один сервер: создает клиента и возвращает конфиг."""
    async with handler:
        try:
            # Создаем клиента
            uid = await handler.add_client_vless(email)
            if not uid:
                print(f"Не удалось создать клиента на {handler.panel_url}")
                return None

            # Получаем конфигурацию
            config = await handler.get_conf_user_vless(email)
            if not config:
                print(f"Не удалось получить конфиг для {email} на {handler.panel_url}")
                return None

            return config
        except Exception as e:
            print(f"Ошибка на сервере {handler.panel_url}: {str(e)}")
            return None


async def main():
    servers = [
        {"url": "https://serv1.myquickcloud.ru:40771/24l6Jo2YjXSjyCH/", "user": "RoWaVjRRw5", "pass": "sb5f0bdGTa"},
        {"url": "https://serv2.myquickcloud.ru:14445/Zp7B4tpK7xKRlTY/", "user": "ca3Muv3SiD", "pass": "zEhtE5ODKS"},
        {"url": "https://serv2.myquickcloud.ru:14445/Zp7B4lTY/", "user": "ca3Muv3SiD", "pass": "zEhtE5ODKS"},
    ]

    email = f"user{random.randint(1000, 9999)}@example.com"

    print(f"Создаем мультиконфиг для {email} на {len(servers)} серверах...")
    multiconfig = await create_multiconfig(servers, email)
